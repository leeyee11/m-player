...
function createWindow () {
  // 创建浏览器窗口。
  	win = new BrowserWindow({width:960, height: 640,frame:false})

  	const listPanelMenu = new Menu();

  	listPanelMenu.append(new MenuItem({label:'添加歌曲',
  		click(){
  	...
  	listPanelMenu.append(new MenuItem({ label:'修改默认歌曲文件夹',
      click(){
        dialog.showOpenDialog({
          properties: ['openDirectory']
        },(dir)=>{
          if(dir.length>0){
            const path=dir[0].replace(/\\/g,"\/");
